...
import { ScrollView, TouchableOpacity, StyleSheet, ActivityIndicator, Image } from 'react-native'
...
export default function ProductDetailScreen() {
  ...
  if (loading) {
    return (
      <ThemedView style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <Stack.Screen
          options={{ title: 'Cargando...', headerStyle: { backgroundColor: '#1A5276' }, headerTintColor: '#FFF' }}
        />
  ...
  if (error || !product) {
    return (
      <ThemedView style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <Stack.Screen
          options={{ title: 'Error', headerStyle: { backgroundColor: '#1A5276' }, headerTintColor: '#FFF' }}
        />
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.background }]}>
      <Stack.Screen
        options={{ title: product.name, headerStyle: { backgroundColor: '#1A5276' }, headerTintColor: '#FFF' }}
      />
      <Image source={{ uri: product.image }} style={styles.image} />
      <ThemedView style={styles.content}>
        <Animated.Text style={styles.category} sharedTransitionTag={`${tag(id)}-category`}>
          {product.category}
        </Animated.Text>
        <Animated.Text style={styles.name} sharedTransitionTag={`${tag(id)}-name`}>
          {product.name}
        </Animated.Text>
        ...
        <Animated.View style={styles.priceRow} sharedTransitionTag={`${tag(id)}-row`}>
          <Animated.Text style={styles.price} sharedTransitionTag={`${tag(id)}-price`}>
          ...
          <ThemedView style={styles.benefitsSection}>
            ...
            {product.benefits.map((b, i) => (
              <ThemedView key={i} style={styles.benefitRow}>
                <ThemedText themeColor="callToActionButton" style={styles.benefitDot}>
                  •
                </ThemedText>
        ...
        <TouchableOpacity
          style={[
            styles.addBtn,
            { backgroundColor: theme.callToActionButton },
            !product.isAvailable() && styles.addBtnDisabled,
          ]}
...
const styles = StyleSheet.create({
  ...
  benefitsSection: { borderRadius: 12, padding: 16, marginBottom: 20, elevation: 1 },
  ...
  benefitDot: { fontSize: 18, marginRight: 8, opacity: 0.8 },
